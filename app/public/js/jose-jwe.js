...
JoseJWS.Signer.prototype.addSigner = function(rsa_key, key_id, aad, header) {
  var that = this,
    key_promise = rsa_key instanceof CryptoKey ? new Promise(function(resolve) {
      resolve(rsa_key);
    }) : Jose.Utils.importRsaPrivateKey(rsa_key, that.cryptographer.getContentSignAlgorithm(), "sign"),
    kid_promise;
...
JoseJWS.Verifier.prototype.verify = function() {
  ...
  var sig = that.signatures[kid]; // *** crutch *** Added this line to save link to the signature
  ...
  for (var kid in that.key_promises) {
    if (that.key_promises.hasOwnProperty(kid)) {
      // *** that.signatures[kid] undefined here
      promises.push(that.cryptographer.verify(sig.aad, that.payload, sig.signature, that.key_promises[kid], kid));
    }
  }
